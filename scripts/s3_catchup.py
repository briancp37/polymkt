...
def fetch_batch(client: httpx.Client, last_timestamp: int, batch_size: int = 1000) -> list[dict]:
    """Fetch a batch of orderFilledEvents from Goldsky using httpx (no internal retries)."""
    query = {
        "query": f"""query {{
            orderFilledEvents(
                orderBy: timestamp
...
def run_catchup(
    ...
    # Initialize - httpx has NO internal retries, we control everything
    client = httpx.Client()
    ...
    while True:
        ...
        # Rate limit: delay BEFORE each request (not after)
        # This ensures we never burst, even on first request or after retries
        if batch_count > 0:  # Skip delay on very first request
        ...
        while retry_count < max_fetch_retries:
            try:
                ...
                print(f"Fetch error: HTTP {e.response.status_code}, retry {retry_count}/{max_fetch_retries} in {delay}s...")
